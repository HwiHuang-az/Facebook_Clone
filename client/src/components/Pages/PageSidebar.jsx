...
const PageSidebar = ({ activeTab, onTabChange, onCreatePage, searchQuery, onSearchChange }) => {
    ...
    useEffect(() => {
        const fetchLikedPages = async () => {
            try {
                const res = await api.get('/pages/liked');
                if (res.data.success) {
    ...
    return (
        <div className="w-full lg:w-90 bg-white shadow-sm h-full overflow-y-auto p-4 border-r flex flex-col pt-4 font-segoe">
            ...
            <div className="space-y-1 mb-6">
                ...
                <button
                    ...
                    className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg font-semibold transition-colors ${isActive('likes') ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-100'}`}
                >
                    <div className={`p-2 rounded-full ${isActive('likes') ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}>
            ...
            <div className="border-t pt-4">
                <div className="flex justify-between items-center mb-4 px-1">
                    <h2 className="font-bold text-gray-900 border-b-0">Trang bạn đã thích</h2>
                    <button
                        onClick={() => onTabChange?.('likes')}
                        ...
                        likedPages.slice(0, 8).map((page) => (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="font-bold text-gray-900 truncate text-sm">{page.name}</h3>
                                    <p className="text-[11px] text-gray-500 truncate font-medium">Lượt thích: {page.likesCount || 0}</p>
                                </div>
                            </Link>
                        ))
                    ) : (
